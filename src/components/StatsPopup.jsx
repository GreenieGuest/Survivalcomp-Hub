import {Container, ColorPicker, parseColor, Button, HStack, Input, Popover, Portal, Text, VStack, NumberInput} from "@chakra-ui/react";
...
function convertColor(input) {
    ...
    // detect if rgb or keyword and convert to hex
    if (/^[0-9A-F]{3,8}$/i.test(cleanInput)) {
        return input
    } else {
        return convert.keyword.hex(cleanInput);
    }
}
...
const StatsCell = ({currentProfile, setPlayerList}) => {
    ...
    const [str, setStr] = useState(toString(currentProfile.str || 3)); // Convert to string because of Chakra UI constraints
    const [dex, setDex] = useState(toString(currentProfile.dex || 3));
    const [int, setInt] = useState(toString(currentProfile.int || 3));
    const [soc, setSoc] = useState(toString(currentProfile.soc || 3));
    //Sim-specific stats
    ...
    return (
        <Container pr={2}>
            <Popover.Root open={open} onOpenChange={(e) => setOpen(e.open)}>
                <Popover.Trigger className={'manual-button'} color={currentProfile.color}>
                    ...
                    <Popover.Positioner>
                        <Popover.Content>
                            ...
                            <Popover.Body>
                                ...
                                <Popover.Title fontWeight="bold" color={currentProfile.color} mb={1}>{`Editing ${currentProfile.name}'s Profile`}</Popover.Title>
                                    <Button onClick={() => {
                                        setOpen(false);
                                    }} size={'sm'} h={'38px'} variant={'outline'} colorPalette={"red"}>Close</Button>
                                </HStack>
