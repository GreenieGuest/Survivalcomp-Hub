...
const SimulationSelector = ({ simulation, setSimulation }) => {
  ...
  const simulations = createListCollection({
    items: [
      ...
      { label: "Algicosathlon", value: "algo", description: "Athletes compete in challenges for points. The competitor with the least points is eliminated each round.", icon: <MdOutlineLeaderboard /> },
    ],
  })
  ...
  return (
    ...
    <Select.Root collection={simulations} onChange={(e) => setSimulation(e.target.value)} size="sm" width="320px">
      <Select.HiddenSelect />
      <Select.Label>Select simulation (NOT YET IMPLEMENTED)</Select.Label>
      <Select.Control>
        <Select.Trigger>
